  ...
  async checkExpiringLicenses(): Promise<{
    ...
    const alerts = expiringSoon.map((license) => {
      const daysUntilExpiry = Math.ceil(
        (license.expiryDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000),
      );
    ...
    if (expired.length > 0) {
      await this.licenseRepository.update(
        { id: expired.map((l) => l.id) as any },
        { status: LicenseStatus.EXPIRED },
      );
    }

    this.logger.log(
      `Found ${expired.length} expired licenses and ${expiringSoon.length} expiring soon`,
    );
