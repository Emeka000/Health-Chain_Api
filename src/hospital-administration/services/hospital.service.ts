...
export class HospitalService {
  ...
  async create(createHospitalDto: CreateHospitalDto): Promise<Hospital> {
    ...
    if (existingHospital) {
      throw new BadRequestException(
        'Hospital with this name or license number already exists',
      );
    }
  ...
  async findAll(
    query: FilterQuery,
  ): Promise<{ data: Hospital[]; pagination: any }> {
    const {
      page = 1,
      limit = 10,
      sortBy = 'createdAt',
    ...
    const [hospitals, total] =
      await this.hospitalRepository.findAndCount(options);
  ...
  async getDashboardStats(hospitalId: string): Promise<DashboardStats> {
    ...
    const occupiedBeds =
      hospital.beds?.filter((bed) => bed.status === 'occupied').length || 0;
    const occupancyRate = totalBeds > 0 ? (occupiedBeds / totalBeds) * 100 : 0;

    // Get staff statistics
    const totalStaff = hospital.staff?.length || 0;
    const activeStaff =
      hospital.staff?.filter((staff) => staff.status === 'active').length || 0;
