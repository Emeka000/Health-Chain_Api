...
export class MedicalStaffService {
  ...
  async createDoctor(createDoctorDto: CreateDoctorDto): Promise<Doctor> {
    ...
    if (createDoctorDto.licenses) {
      for (const license of createDoctorDto.licenses) {
        ...
        if (existingLicense) {
          throw new ConflictException(`License number ${license.licenseNumber} already exists`)
        }
      }
    }
  ...
  async getDoctor(id: string): Promise<Doctor> {
    const doctor = await this.doctorRepository.findOne({
      where: { id },
      relations: ["department", "specialties", "licenses", "schedules", "performanceMetrics", "continuingEducation"],
    })
  ...
  async renewLicense(licenseId: string, newExpiryDate: Date): Promise<MedicalLicense> {
  ...
  async recordContinuingEducation(createEducationDto: CreateContinuingEducationDto): Promise<CreateContinuingEducationDto> {
    const education = this.educationRepository.create(createEducationDto)
    return this.educationRepository.save(education)
  }

  async getDoctorEducationCredits(doctorId: string): Promise<CreateContinuingEducationDto[]> {
  ...
  async getEducationCompliance(): Promise<{
    ...
    return {
      ...
      summary: {
        ...
        complianceRate: doctors.length > 0 ? (compliant.length / doctors.length) * 100 : 0,
      },
    }
  }
