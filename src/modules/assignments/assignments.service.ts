...
export class AssignmentsService {
  ...
  async create(createAssignmentDto: CreateAssignmentDto): Promise<Assignment> {
    const assignment = this.assignmentsRepository.create({
      ...
      endTime: createAssignmentDto.endTime
  ...
  async update(
    ...
    if (updateAssignmentDto.startTime) {
      updateAssignmentDto.startTime = new Date(
        updateAssignmentDto.startTime,
      ) as any;
    }

    if (updateAssignmentDto.endTime) {
      updateAssignmentDto.endTime = new Date(
        updateAssignmentDto.endTime,
      ) as any;
    }
