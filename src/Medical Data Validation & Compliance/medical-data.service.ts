...
import {
  ...
  MedicalEmergencyException,
} from './medical-exception.filter';
...
export class MedicalDataService {
  ...
  private async validateMedicalCodes(
    data: CreateMedicalDataDto,
  ): Promise<void> {
    ...
    for (const medicalCode of data.medicalCodes) {
      ...
      const icd10Validation = await this.codeValidator.validateICD10Code(
        medicalCode.icd10Code,
      );
      ...
      // Validate CPT code
      const cptValidation = await this.codeValidator.validateCPTCode(
        medicalCode.cptCode,
      );
  ...
  private async handleEmergencyLevel(
    data: CreateMedicalDataDto,
    userId: string,
    ipAddress: string,
  ): Promise<void> {
    if (data.emergencyLevel === EmergencyLevel.CRITICAL) {
      ...
      this.logger.error(
        `CRITICAL MEDICAL EMERGENCY - Patient: ${data.patientId}`,
      );
