...
export class PatientsService {
  ...
  async create(createPatientDto: CreatePatientDto): Promise<Patient> {
    const patient = this.patientsRepository.create({
      ...
      dischargeDate: createPatientDto.dischargeDate ? new Date(createPatientDto.dischargeDate) : null,
    })
  ...
  async update(id: string, updatePatientDto: UpdatePatientDto): Promise<Patient> {
    ...
    if (updatePatientDto.admissionDate) {
      updatePatientDto.admissionDate = new Date(updatePatientDto.admissionDate) as any
    }

    if (updatePatientDto.dischargeDate) {
      updatePatientDto.dischargeDate = new Date(updatePatientDto.dischargeDate) as any
    }
