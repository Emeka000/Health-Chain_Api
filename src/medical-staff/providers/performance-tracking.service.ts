...
export class PerformanceTrackingService {
  ...
  async createPerformanceMetric(createMetricDto: CreatePerformanceMetricDto): Promise<PerformanceMetric> {
  ...
  async getDoctorPerformance(
    ...
    const query = this.metricRepository.createQueryBuilder("metric").where("metric.doctorId = :doctorId", { doctorId })
    ...
    const metrics = await query.orderBy("metric.recordedDate", "DESC").getMany()

    // Calculate summary statistics
    const summary = this.calculatePerformanceSummary(metrics)
    const trends = await this.calculatePerformanceTrends(doctorId, startDate, endDate)
  ...
  async getDepartmentPerformanceSummary(departmentId?: string): Promise<{
    ...
    const departmentSummaries = await Promise.all(
      departments.map(async (department) => {
        const activeDoctors = department.doctors.filter((d) => d.status === "ACTIVE")
        const departmentMetrics = activeDoctors.flatMap((d) => d.performanceMetrics || [])

        const averagePerformance = this.calculateDepartmentAverages(departmentMetrics)
        const topPerformers = this.identifyTopPerformers(activeDoctors)
        const improvementAreas = this.identifyImprovementAreas(departmentMetrics)
  ...
  private calculatePerformanceSummary(metrics: PerformanceMetric[]): {
    ...
    const satisfactionMetrics = metrics.filter((m) => m.metricType === MetricType.PATIENT_SATISFACTION)
    const treatmentMetrics = metrics.filter((m) => m.metricType === MetricType.TREATMENT_SUCCESS_RATE)
    const consultationMetrics = metrics.filter((m) => m.metricType === MetricType.CONSULTATION_TIME)
    const patientCountMetrics = metrics.filter((m) => m.metricType === MetricType.PATIENT_COUNT)
    ...
    const treatmentSuccessRate =
      treatmentMetrics.length > 0 ? treatmentMetrics.reduce((sum, m) => sum + m.value, 0) / treatmentMetrics.length : 0

    const averageConsultationTime =
      consultationMetrics.length > 0
        ? consultationMetrics.reduce((sum, m) => sum + m.value, 0) / consultationMetrics.length
        : 0

    const totalPatients = patientCountMetrics.reduce((sum, m) => sum + m.value, 0)

    // Calculate composite quality score
    const qualityScore = patientSatisfactionAvg * 0.4 + treatmentSuccessRate * 0.6
  ...
  private async calculatePerformanceTrends(
    ...
    const currentPeriodStart = startDate || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) // 30 days ago
  ...
  private calculateTrendPercentage(previousValue: number, currentValue: number, lowerIsBetter = false): number {
  ...
  private identifyTopPerformers(doctors: Doctor[]): Doctor[] {
    ...
    return doctors
      .map((doctor) => ({
        doctor,
        qualityScore: this.calculatePerformanceSummary(doctor.performanceMetrics || []).qualityScore,
      }))
  ...
  private calculateOverallSummary(departmentSummaries: any[]): {
    ...
    const totalDoctors = departmentSummaries.reduce((sum, dept) => sum + dept.doctorCount, 0)

    const averageQualityScore =
      departmentSummaries.length > 0
        ? departmentSummaries.reduce((sum, dept) => sum + dept.averagePerformance.qualityScore, 0) /
    ...
    const topDepartment =
      departmentSummaries.length > 0
        ? departmentSummaries.reduce((top, dept) =>
            dept.averagePerformance.qualityScore > top.averagePerformance.qualityScore ? dept : top,
          ).department.name
        : "N/A"
